...
\begin{algorithm}
    \KwData{
      ...
      MAX\_DEPTH \\* the number of variables \\*
...
We could implement a hypercube shuffle for Spark and then choose any \textsc{WCOJ} to run on all partitions.
...
of this thesis (\ref{sec:shares-proof}). % TODO mention simulation
As result of this proof, we decide to not physically partition our data but to duplicate it to all workers and seek for alternative
...
Therefore, we treat it in the related work section of this thesis (\ref{subsec:wcoj-timely-data-flow}).

\subsubsection{Shares}
Shares partitions the input relationships for a multi-way join over \textit{w} worker nodes, such that, all tuples, which could be joined,
...
arrive on the same worker after one shuffle round.
We first explain how to organize the workers in a hypercube and then how to map tuples to these workers.
% TODO find best hypercube configuation is not straigthforward.
Finally, we note that this scheme is communication cost optimal and the implications of this proof.
...
They have the size of 3, 3 and 2 for $a, b$ and $c$ respectively.
It can be seen as a possible configuration for the triangle query $triangle(a, b, c) \leftarrow R(a, b), S(b, c), T(a, c)$ with
12 workers.
...
We discuss it in later in~\cref{subsubsubsec:hypercube-configuration}.
...
We choose a hash function for each join variable which maps its values in the range of $(0..p_v)$.
...
The blue, green and red tuple in $S, R$ and $T$ form a triangle.
...
\subsubsubsection{Finding the best hypercube configuration} \label{subsubsubsec:hypercube-configuration}
% TODO

\subsubsection{Shares is worst-case communication-cost optimal}
Shares, as described above, is shown to be worst-case optimal in its communication costs in MapReduce like systems for
n-ary joins with one shuffle round.
...
tuples).
Finally, while first experiments with SharesSkew exists~\textit{shares-skew}, we are not aware of an extensive study verifying it is possible
...
Indeed, the authors of multiple Shares related papers raise the same question as future work~\cite{shares-skew-proof}.
They are able to answer this question for specific joins in~\cite{shares-skew-proof}, e.g. chain-joins and cycle-joins.
Later, they present an algorithm which is multi-round optimal for acyclic queries~\cite{gym}.

The discussions about multi-round optimal partitioning schemes are rather theoretical.
To the best of our knowledge, no implementation has been published.
Also, they have not been shown optimal for general conjunctive join queries but only for special cases.
However, not for clique joins which are important class of joins in our thesis.
Additionally, they add additonal complexity to the query optimizer, e.g. they require the input query to be represented as
...
praxis~\cref{subsubsubsect:hypercube-configuration}.
Again, we leave it to future research to investigate the practical application of these algorithms to graph pattern matching.
The most interesting paper in this direction is~\cite{shares-binary}.
It develops a multi-round algorithm for n-ary joins on binary relationships as the edge relationship of a graph.
...
\subsubsubsection{Shares scalibility proof}\label{subsubsubsec:shares-proof}
