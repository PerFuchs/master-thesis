...
The query optimizer of Spark, Catalyst, is the component of Spark we integrate our \textsc{WCOJ} with; its structure is explained
...
When started, the driver program acquires resources from the \textit{cluster manager} to start its executors.
...
Spark allows the user to choose its cluster manager to manage resources in the cluster.
...
a standalone mode where it provides its own cluster manager functionality.
...
The exchange operator builds a hashtable from its input operator and makes it available as a broadcast variable to all executors of
...
This is a good example of a strategy that translates a single logical operator into two physical operators.
The other change to the logical plan is that the count aggregation has been broken up into a partial aggregation directly after the
last join, an exchange reorganizing all partial counts into a single partition and a second aggregation over that partition to calculate
the total count.
...
One should note that \textit{structured query execution} does not execute the query: the result is an RDD which is a none
...
After initialization, they can be accessed by every task.
When they are not cached the task acquires the value of the variable from the driver program or other workers, deserializes it and
adds it to the cache shared by all tasks on the machine.
Spark guarantees that each broadcast variable is sent only once to each executor and allows it to be spilled to disk if it is not
