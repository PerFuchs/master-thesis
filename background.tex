...
For instance, the triangle query cannot be solved in less than $\mathcal{O} (N^2)$ of the edge table size, while the AGM bound
...
We refer the reader to these papers for further infomation.
...
Both algorithms have been shown to be instances of a single algorithm, the `Generic Join', in 2013 Ngo et al.~\cite{skew-strikes-back}.

We choose to use the Leapfrog Triejoin as the basis for our work.
This is for two reasons.
First, because the Leapfrog Triejoin is the only worst-case join algorithm that has been implemented and benchmarked numerous times
in widely different settings, e.g. Oxford course work, published research and a commercial database system~\cite{leapfrog,andreas,olddog,myria,ammar2018distributed,leapfrog-triejoin-schroeder}.
We are not aware of any implementations of the NPRR join.
To the best of our knowledge, the generic join has been used in two published papers~\cite{amin,timely-dataflow}, we
discuss these in our related work section (~\ref{subsec:wcoj-timely-data-flow} and \ref{subsec:wcoj-binary-joins}).
Second, the \textsc{LFTJ} has been integrated into a distributed MapReduce like system called Myria which is similar to Spark.
This implementation has strong influence on our work and is discussed in~\cref{subsec:myria}.
