...
\begin{figure}
    \includesvg[width=0.5\textwidth]{hypercube.svg}
    \caption{}
    \label{fig:hypercube}
\end{figure}
...
% Optimiality
This scheme is proven to be communication cost optimal for n-way joins in any MapReduce like environment in~\cite{shares}.
This implies that it is not possible to find a partitioning scheme that replicates less data, given that we shuffle/communicate only once.
...
% configurations for skewed values. connection with SharesSkew?
...
% TODO multiple shuffle rounds, semih's research statement, shuffles are very expensive in Spark.
