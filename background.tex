...
For this, we use the method described in above and used in~\cite{myria-detailed}.
...
$size1(r)$ and $size2(r)$ be the size of the first respectively second hypercube dimesion for the two variables in atom $r$.
...
binary and differently distributed trials succeed, under the condition that the $i$'th trial succeeds with a probability of $u_i$.
...
This is due to the fact that each tuple binds only two out of all variables.
...
By this observation and the fact that hypercube shuffling is a optimal scheme (see last paragraph),
