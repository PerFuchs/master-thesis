...
\begin{algorithm}[H]
    \KwData{\\
      \textit{iters} sorted array of TrieIterators \\
      \textit{p} index of the smallest iterator
    }
   \KwResult{Either \textit{atEnd} is true or \textit{key} is set to next key of intersection}
...
\begin{algorithm}
    \KwData{
      \textit{depth} the index of the variable to find a binding for, ranges from -1 to \#variables - 1\\
      \textit{bindings} array holding the current variable bindings or $-1$ for no binding \\
      \textit{MAX\_DEPTH} the number of variables - 1 \\
      \textit{action} state of the state machine
    }
