...
In short, this bound proves that traditional, binary join plans perform asymptotically worse than theoretical possible
...
In the next paragraph, we discuss different algorithms matching the AGM bound which are called worst-case optimal joins.
...
Next, we compare these two algorithms in ease of implementation.
...
It consists of four contributions which build upon each other but could be useful on its own.
...
As a result of this investigation, we decide to not physically partition our data but to duplicate it to all workers and seek for alternative
