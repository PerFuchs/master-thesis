...
group.
These machines feature 4 Intel Xeon E5-4657Lv2 processors with 12 cores each and hyperthreading of 2 (96 threads)
...
Towards this goal, we run \textit{Graph\textsc{WCOJ}} on datasets of different size namely \textit{Twitter},
\textit{LiveJournal} and \textit{Orkut}.
We compare three partitioning schemes: \textit{Shares} and \textit{Workstealing}.
These are the two most promising schemed identified in \cref{ssec:partitionings}.
The experiment is performed on \textit{3-clique} and \textit{5-clique}.
\textit{3-clique} is the smallest of our queries.
Therefore, it is most difficult to scale.
\textit{5-clique} takes much longer than \textit{3-clique}.
Hence, it shows how query size influences the scaling.
Also, it increases the job size for the \textit{Workstealing} partitioning scheme.
...
\begin{table}
    \centering
    \input{generated/twitter-scaling.tex}
    \caption{g}    \label{t}
    % TODO align and remove headers
\end{table}
