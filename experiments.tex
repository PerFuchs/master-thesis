...
The optimum for LFTJ is around 200 while GraphWCOJ shows the best performance when it executes only linear
searches;
...
\begin{figure}
    \centering
    \subfloat[SNB-1\label{fig:linear-search-threshold-snb}]{\includesvg[width=0.5\textwidth]{linear-search-threshold-snb}}
    \subfloat[Twitter\label{fig:linear-search-threshold-twitter}]{\includesvg[width=0.5\textwidth]{linear-search-threshold-twitter}}
...
From this experiment, we conclude that the optimal threshold for \textsc{LFTJ} is 200 and 1600
for GraphWCOJ.
