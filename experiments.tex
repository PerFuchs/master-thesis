...
calls to the underlying iterators because I start the intersection with the
smallest list.
So it does not improve computation because of better caching behaviour as I used to believe.

Then I experimented with the linear search thresholds and found that the algorithm gets faster
...
Therefore, the optimizations don't look great anymore.
...
The \texttt{Amazon} datasets show quite similar behaviour overall queries.
...
from the materialization of the intersections, i.e. clique queries.
...
% TODO fix legend of livejournal scaling to include the batch line
...
\begin{table}
    ...
    % TODO did I explain batch size? in the work-stealing chapter.
...
% TODO write Orkut dataset or only Orkut everywhere
