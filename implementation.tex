...
% TODO is that the write place?
It is not in scope of this work to integrate WCOJ's into the SQL parser of Spark.
...
%It is established that WCOJ are amendable to cyclic queries but not how strongly to which kind of cyclic query, e.g. dense (clique) or
%sparse (cycle).
...
To support the Trieiterator interface, we need to materialize the input RDD's for \texttt{WCOJExec}.  % TODO footnote the Spark novice
