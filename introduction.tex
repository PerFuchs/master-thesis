...
We aim to integrate a scalable, \textsc{WCOJ} algorithm in Spark which is used by some modern graph engines~\cite{caps,gcore,graphframe}.
...
Graph pattern matching is fundamental to analytical graph analysis workloads~\cite{twitter-diamonds,fraud-detection,flake2002,
...
However, shuffling is an expensive operation, involving disk writes and reads, which should be avoided if possible.
