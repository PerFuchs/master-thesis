...
Fot this, the work is not statically partitioned before-hand but organized in many smaller tasks which can be
...
We discuss how the design can be extended Spark's \textit{cluster-mode} in future work (\cref{subsec:cluster-mode}) and
...
We choose to the define a task as the work necessary to find all possible tuples for a single binding of the first join variable.
...
This queue can be accesses by each Spark thread.
...
Moreover, we cannot guarantue to construct an equal partitioning over multiple runs of the same query.
